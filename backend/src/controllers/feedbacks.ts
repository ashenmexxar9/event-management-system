...
export const getFeedbacksForEvent = async (req: Request, res: Response) => {
  try {
...
export const getFeedbacksByUser = async (req: Request, res: Response) => {
  try {
    ...
    const feedback = await getAsync(
      'SELECT * FROM feedbacks WHERE event_id = ? AND user_id = ?',
      [eventId, req.user.id]
    );
...
export const createFeedback = async (req: Request, res: Response) => {
  try {
    ...
    const { eventId } = req.params;
    const { rating, comment } = req.body as Feedback;
    ...
    const id = uuidv4();
    await runAsync(
      `INSERT INTO feedbacks (id, event_id, user_id, rating, comment)
       VALUES (?, ?, ?, ?, ?)`,
      [id, eventId, req.user.id, rating, comment || null]
    );

    const feedback = await getAsync('SELECT * FROM feedbacks WHERE id = ?', [id]);
...
export const updateFeedback = async (req: Request, res: Response) => {
  try {
    ...
    const feedback = await getAsync('SELECT * FROM feedbacks WHERE id = ?', [feedbackId]);
    ...
    await runAsync(
      `UPDATE feedbacks SET rating = ?, comment = ?, updated_at = CURRENT_TIMESTAMP
       WHERE id = ?`,
      [rating || feedback.rating, comment !== undefined ? comment : feedback.comment, feedbackId]
    );

    const updated = await getAsync('SELECT * FROM feedbacks WHERE id = ?', [feedbackId]);
    res.json(updated);
  } catch (error) {
...
export const deleteFeedback = async (req: Request, res: Response) => {
  try {
    ...
    await runAsync('DELETE FROM feedbacks WHERE id = ?', [feedbackId]);
